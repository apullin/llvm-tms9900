// TMS9900 patched copy of libcxx/include/__functional/hash.h
// Changes from upstream LLVM 20:
//   - Removed static_assert(sizeof(size_t) >= sizeof(unsigned long)) in
//     hash<unsigned long> specialization (TMS9900: size_t=16bit, ulong=32bit)
//   - Added __murmur2_or_cityhash<_Size, 16> specialization for 16-bit size_t
//
//===----------------------------------------------------------------------===//
//
...
struct _LIBCPP_TEMPLATE_VIS hash<unsigned long> : public __unary_function<unsigned long, size_t> {
  _LIBCPP_HIDE_FROM_ABI size_t operator()(unsigned long __v) const _NOEXCEPT {
    // TMS9900: static_assert removed -- sizeof(size_t)=2 < sizeof(unsigned long)=4
    // Truncation to 16-bit hash is acceptable.
