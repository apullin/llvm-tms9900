 ...
 *
 * Optimizations over life.c:
 *   1. Simplified Life rule: bit1 & ~bit2 & ~bit3 & (bit0 | mC)
 *   2. Word-level active_tiles check (single uint16_t load)
...
/*
 * OPT 1: Simplified Life rule.
 *
 * Original:  n3 | (n2 & mC)
 *   where n2 = ~bit0 & bit1 & ~bit2 & ~bit3  (count == 2)
 *         n3 =  bit0 & bit1 & ~bit2 & ~bit3  (count == 3)
 *
 * Simplified: bit1 & ~bit2 & ~bit3 & (bit0 | mC)
 *   Proof: Let X = bit1 & ~bit2 & ~bit3.
 *   n3 | (n2 & mC) = (bit0 & X) | (~bit0 & X & mC) = X & (bit0 | mC).
 */
...
static void __attribute__((noinline)) step_board(uint8_t (*src)[ROW_BYTES],
    ...
    for (y = LIFE_REGION_Y0; y <= LIFE_REGION_Y1; y++) {
        ...
        /* OPT 3: Sliding window - cache center and right columns. */
        uint16_t sw_uC, sw_uR;
        uint16_t sw_mC, sw_mR;
        uint16_t sw_dC, sw_dR;
        uint16_t need_reload = 1;

        for (wx = 0; wx < LIFE_REGION_WORDS; wx++) {
            uint16_t tile_x = (uint16_t)(wx << 1);
            /* OPT 2: Word-level active tiles check. */
            if (*(u16_alias *)&active_tiles[ty][tile_x] == 0) {
                need_reload = 1;
                continue;
            }
            ...
            if (need_reload) {
                /* First word after gap: full 9-value load. */
                ...
                /* OPT 3: Slide from previous iteration. */
                uL = sw_uC;
                uC = sw_uR;
                mL = sw_mC;
                mC = sw_mR;
            ...
            uR = row_prev[wx_right];
            mR = row[wx_right];
            dR = row_next[wx_right];
